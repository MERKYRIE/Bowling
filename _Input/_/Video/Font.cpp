...
namespace NAdvancedEngineArchitecture::NVideo
{
    ...
    std::shared_ptr<const CCharacter> CFont::OAccessDigit(char PCode) const
    {
        return(FDigits[static_cast<std::int64_t>(PCode) - static_cast<std::int64_t>('0')]);
    }
    std::shared_ptr<const CCharacter> CFont::OAccessUppercase(char PCode) const
    {
        return(FUppercases[static_cast<std::int64_t>(PCode) - static_cast<std::int64_t>('A')]);
    }
    std::shared_ptr<const CCharacter> CFont::OAccessLowercase(char PCode) const
    {
